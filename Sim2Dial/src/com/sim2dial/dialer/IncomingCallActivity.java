...
import com.sim2dial.dialer.R.bool;
import com.sim2dial.dialer.R.drawable;
...
public class IncomingCallActivity extends Activity implements LinphoneOnCallStateChangedListener, LinphoneSliderTriggered {
	...
	private void answer() {
		...
		if (getResources().getBoolean(R.bool.only_display_username_if_unknown)) {
			//if(callstore){
				LinphoneActivity.instance().storeHistory(address.getDisplayName(),address.getUserName(),"", "Incoming");
				callstore=false;
			//}
		} else {
			//if(callstore){
				LinphoneActivity.instance().storeHistory(address.getDisplayName(),address.asStringUriOnly(),"", "Incoming");
				callstore=false;
			//}
		}
