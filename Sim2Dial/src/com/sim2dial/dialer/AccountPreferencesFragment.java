...
import com.sim2dial.dialer.R.xml;

import com.sim2dial.dialer.compatibility.Compatibility;
import com.sim2dial.dialer.ui.PreferencesListFragment;
...
public class AccountPreferencesFragment extends PreferencesListFragment
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		/*
		 * n = getArguments().getInt("Account", 0); key = getAccountNumber(n);
		 */manageAccountPreferencesFields(screen);

		hidePreferenceCategory(R.string.pref_acc_advanced_key);
	}
	...
	OnPreferenceChangeListener	preferenceChangedListener	= new OnPreferenceChangeListener()
															{
																...
																public boolean onPreferenceChange(Preference preference, Object newValue)
																{
																	...
																	if (preference.toString().contains("Password*"))
																	{
	...
	void hidePreference(int str)
	{
		Preference screen = findPreference(getString(str));
		screen.setLayoutResource(R.layout.hidden);
	}
	...
	private void manageAccountPreferencesFields(PreferenceScreen parent)
	{
		...
		password.setOnPreferenceChangeListener(preferenceChangedListener);
		...
		account.removePreference(domain);
		...
		delete.setOnPreferenceClickListener(new OnPreferenceClickListener()
		{
			public boolean onPreferenceClick(Preference preference)
			{
				...
				for (int i = n; i < nbAccounts - 1; i++)
				{
					...
					editor.putBoolean(getString(R.string.pref_enable_outbound_proxy_key) + getAccountNumber(i),
							prefs.getBoolean(getString(R.string.pref_enable_outbound_proxy_key) + getAccountNumber(i + 1), false));
					editor.putBoolean(getString(R.string.pref_disable_account_key) + getAccountNumber(i),
							prefs.getBoolean(getString(R.string.pref_disable_account_key) + getAccountNumber(i + 1), false));
				}
	...
	public void onPause()
	{
		...
		int n = Engine.getPref().getInt(getString(R.string.pref_extra_accounts), 1);
		String keyUsername = getString(R.string.pref_username_key) + (n - 1 == 0 ? "" : Integer.toString(n - 1));

		if (Engine.getPref().getString(keyUsername, "").equals(""))
		{
			// If not, we suppress it to not display a blank field

			Engine.getEditor().putInt(getString(R.string.pref_extra_accounts), n - 1).commit();
		}
