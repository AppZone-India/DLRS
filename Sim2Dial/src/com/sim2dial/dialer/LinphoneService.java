...
import com.sim2dial.dialer.LinphoneManager.NewOutgoingCallUiListener;
...
public final class LinphoneService extends Service implements LinphoneServiceListener
{
	...
	private boolean					mTestDelayElapsed	= true;			// no
																			// timer
	...
	public void onGlobalStateChanged(final GlobalState state, final String message)
	{
		if (state == GlobalState.GlobalOn)
		{
			...
			mHandler.postDelayed(new Runnable()
			{
				public void run()
				{
					if (guiListener() != null) guiListener().onGlobalStateChangedToOn(message);
				}
			}, 50);
		}
	}
	...
	public void onCallStateChanged(final LinphoneCall call, final State state, final String message)
	{
		...
		mHandler.post(new Runnable()
		{
			public void run()
			{
				if (guiListener() != null) guiListener().onCallStateChanged(call, state, message);
			}
		});
	}
	...
	{
		Engine.getEditor().putString(getString(R.string.pref_audio_ringtone), ringtone).commit();
	}

	public void onRingerPlayerCreated(MediaPlayer mRingerPlayer)
	{
		String uriString = Engine.getPref().getString(getString(R.string.pref_audio_ringtone), android.provider.Settings.System.DEFAULT_RINGTONE_URI.toString());
		try
		{
	...
	public void tryingNewOutgoingCallButAlreadyInCall()
	{
		mHandler.post(new Runnable()
		{
			public void run()
			{
				if (guiListener() != null) guiListener().onAlreadyInCall();
			}
		});
	}
	...
	public void tryingNewOutgoingCallButCannotGetCallParameters()
	{
		mHandler.post(new Runnable()
		{
			public void run()
			{
				if (guiListener() != null) guiListener().onCannotGetCallParameters();
			}
		});
	}
	...
	public void tryingNewOutgoingCallButWrongDestinationAddress()
	{
		mHandler.post(new Runnable()
		{
			public void run()
			{
				if (guiListener() != null) guiListener().onWrongDestinationAddress();
			}
		});
	}
